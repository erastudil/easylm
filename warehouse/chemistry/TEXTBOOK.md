...
# chemistry — undergrad textbook

a working book for people who must **name the species, balance the change, then count**.
this file teaches. table values, K, ΔH, hazards → DONT_KNOW / fetch the named door. arithmetic only with the **EasyLM calc** hand. unit conversion only with the **EasyLM units** hand.

**law this book applies:** atoms in = atoms out. charge balances. mole first. state (s)(l)(g)(aq). equilibrium ≠ rate. lab numbers and SDS are a fetch. do not invent a yield, a Ka, or a hazard.

**constants.** G, h, k, N_A, R, F: do not invent digits. fetch the live CODATA value at https://physics.nist.gov/cuu/Constants/ then pass stated numbers to EasyLM calc. SI definitions that are exact by BIPM may be named as definitions (the mole is an SI base unit; N_A is a defining constant — digits still from that NIST page).
...
|---|---|
| what the field is | 1 |
| atoms, isotopes, the table | 2 |
| mole, molar mass, concentration | 3 |
| bonds and shape | 4 |
| balance, limiting reagent, yield | 5 |
| gas, solution, phase | 6 |
| heat of reaction, how fast | 7 |
| K, le chatelier | 8 |
| acids, bases, pH | 9 |
| redox, cells | 10 |
| carbon skeletons, polymers | 11 |
| is this safe | 12 |
| compute / tables | 13 |
| stuck on a problem | 14 |

work order: **why → what → how**. name the species before you write an equation.
...
home: `warehouse/chemistry/TEXTBOOK.md`
